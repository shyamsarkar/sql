-- 612. Shortest Distance in a Plane (Medium, SQL)
...
FROM Point2D p1 JOIN Point2D p2 ON p1.x < p2.x OR (p1.x = p2.x AND p1.y < p2.y);


SELECT ROUND(MIN(
  SQRT(POWER(p1.x - p2.x, 2) + POWER(p1.y - p2.y, 2))
)::numeric, 2) AS shortest
FROM Point2D p1 CROSS JOIN Point2D p2 WHERE p1.x <> p2.x OR p1.y <> p2.y;
